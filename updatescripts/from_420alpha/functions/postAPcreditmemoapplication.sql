...
-- #21155 - Modify currency conversion as at Voucher Doc Date - not current date
  FOR _r IN SELECT apcreditapply_id, apcreditapply_target_apopen_id,
		   apcreditapply_amount AS apply_amountSource,
                   currToCurr(apcreditapply_curr_id, apopen_curr_id,
                              apcreditapply_amount, apopen_docdate) AS apply_amountTarget,
...
--  Calculate Exchange Gain/Loss.  Has to be done here as Vouchers can have different rates (#21156)
    IF (_r.apopen_curr_id = _src.apopen_curr_id) THEN
      IF (_r.apopen_docdate > _src.apopen_docdate) THEN
        _exchGain := _exchGain + (_r.apply_amountSource / _r.apopen_curr_rate - _r.apply_amountSource / _src.apopen_curr_rate) * -1;
      ELSE
        _exchGain := _exchGain + (_r.apply_amountSource / _src.apopen_curr_rate - _r.apply_amountSource / _r.apopen_curr_rate);
      END IF;
    END IF;
